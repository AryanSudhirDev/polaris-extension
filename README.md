...
The script updates `premium-tokens-expiring.json`, validates token counts and duplicates, prints the generated tokens, and deploys with `vercel --prod --yes` only when `--deploy` is passed.
